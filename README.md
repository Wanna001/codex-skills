...
需要区分两件事：GitHub 上设为 **public** 只代表代码公开可见——任何人可以在 GitHub 网页里查看，
也可以按 GitHub 服务条款在平台内 fork；但它**没有**授予别人使用、修改、再分发的许可，
默认状态是"保留所有权利（all rights reserved）"。**public ≠ 开源。**

如果希望别人能自由使用 / 修改 / 分发，需要显式加一个许可证文件，常见选择：

| 许可证 | 适用场景 |
| --- | --- |
| MIT | 最宽松、最短，脚本 / 配置 / skill 类首选 |
| Apache-2.0 | 额外包含专利授权与商标条款，适合企业场景 |
| GPL-3.0 | 要求衍生作品同样开源（copyleft） |
| CC0-1.0 / Unlicense | 放弃尽可能多的权利，近似公共领域 |

选定后在仓库根目录加 `LICENSE`（或 `LICENSE.md`）即可，GitHub 会自动识别并在仓库页展示许可证类型。
